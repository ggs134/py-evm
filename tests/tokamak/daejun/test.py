...
## Daejun VM is used here
from eth.vm.forks.daejun import DaejunVM
...
klass = chains.base.MiningChain.configure(
    __name__='TestChain',
    vm_configuration=(
        (constants.GENESIS_BLOCK_NUMBER, DaejunVM),
    )
)
...
mined_block = chain.mine_block(mix_hash=mix_hash, nonce=nonce)
vm = chain.get_vm()
...
print("BLOCK2 GAS USED : {}".format(block.as_dict()['header'].gas_used))
#why more than 21060? -> https://ethereum.stackexchange.com/questions/8030/not-all-addresses-created-equal